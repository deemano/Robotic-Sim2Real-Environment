...
if physicsClient < 0:
    sys.exit('Failed to connect to the physics server.')
...
ip = '192.168.1.242'  # Replace with your xArm's IP address
table_pos = [0, 0, -0.625]
...
def capture_image(activate):
    if activate:
        ...
        camera_position = [0.7, 0.7, 0.7]  # Camera position in world coordinates
        ...
        renderer = p.ER_BULLET_HARDWARE_OPENGL  # or ER_BULLET_HARDWARE_OPENGL, ER_TINY_RENDERER for CPU-based rendering
...
try:
    ...
    # Before disconnecting, check if we are still connected to the server
